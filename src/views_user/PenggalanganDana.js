		...
		const selectPenggalanganDanaBeasiswa = async () => {
			await fetch('http://localhost:8000/v1/penggalangan/penggalangan_dana/selectPenggalanganDana',
				...
				.then((data) => {
					...
					console.log('penggalangan_dana_id:', data.data.penggalangan_dana_id)
					console.log('judul:', data.data.judul)
							...
							<Card sx={{ width: 360, height: '100%' }}>
								<CardContent>
									...
									<Link
										...
										state={{
											id: identitas,
											judul: info.length > 0 ? info[0].judul : ''
										}}
									>
										<Button variant="contained" sx={{ mt: 2, width: '100%' }}>
											<Typography sx={{ textTransform: 'capitalize' }}>Donasi Sekarang</Typography>
										</Button>
									</Link>
									{
										jumlahDonatur[0]
